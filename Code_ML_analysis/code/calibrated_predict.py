...
calibrated_models_confound_none = {}
calibrated_models_confound_TIV = {}

calibrated_models_confound_none['age'] = joblib.load(
    base_model_dir + 'calibrated_refitted_model_julearn_confound-None_sample-age')

calibrated_models_confound_none['age_TIV'] = joblib.load(
    base_model_dir + 'calibrated_refitted_model_julearn_confound-None_sample-age_TIV')

calibrated_models_confound_TIV['age'] = joblib.load(
    base_model_dir + 'calibrated_refitted_model_julearn_confound-TIV_sample-age')
calibrated_models_confound_TIV['age_TIV'] = joblib.load(
    base_model_dir + 'calibrated_refitted_model_julearn_confound-TIV_sample-age_TIV')

calibrated_models_dict = {
    None: calibrated_models_confound_none, 'TIV': calibrated_models_confound_TIV}
...
models_confound_none['age'] = joblib.load(
    base_model_dir + 'model_julearn_confound-None_sample-age').best_estimator_

models_confound_none['age_TIV'] = joblib.load(
    base_model_dir + 'model_julearn_confound-None_sample-age_TIV').best_estimator_

models_confound_TIV['age'] = joblib.load(
        base_model_dir + 'model_julearn_confound-TIV_sample-age').best_estimator_
models_confound_TIV['age_TIV'] = joblib.load(
    base_model_dir + 'model_julearn_confound-TIV_sample-age_TIV').best_estimator_
...
def save_predictions(data, name):
    for sample in ['age', 'age_TIV']:
        for conf in [None, 'TIV']:
            ...
            calibrated_model = calibrated_models_dict[conf][sample]
            X_prep, *_ = model.preprocess(df_val, y)
            df_predictions['pred'] = calibrated_model.predict(X_prep)
            proba_classes = [f'pred_proba_{int(i)}'for i in model.classes_]
            df_predictions[proba_classes] = calibrated_model.predict_proba(X_prep)
            df_predictions.to_csv(
                f'{base_save_dir}{name}_calibrated_predictions_model_trained_on_confound-{conf}_sample-{sample}.txt',
                index=False)
